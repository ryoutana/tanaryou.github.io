...
function generateSpots() {
    ...
    spots.forEach(spot => {
        ...
        img.style.filter = 'brightness(0.8) hue-rotate(300deg)'; // デフォルトの灰色

        // 当たり判定用の透明な要素を作成
        const hitbox = document.createElement('div');
        hitbox.style.position = 'absolute';
        hitbox.style.width = `${(spot.width * 0.3 / 800) * mapWidth}px`; // 表示されている画像の0.3倍の幅
        hitbox.style.height = `${(spot.width * 0.3 / 800) * mapHeight}px`; // 表示されている画像の0.3倍の高さ
        ...
        hitbox.addEventListener('mouseout', () => {
            if (!img.classList.contains('active')) {
                img.style.filter = 'brightness(0.8) hue-rotate(300deg)'; // 元の灰色に戻す
            }
        });
